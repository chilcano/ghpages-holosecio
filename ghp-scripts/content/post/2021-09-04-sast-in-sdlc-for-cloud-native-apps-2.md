...
Unlike [the previous post](/2021/07/06/sast-in-sdlc-for-cloud-native-apps/), this post will implement a Jenkins Pipeline for a Full Stack Cloud-Native Application. That means, that Jenkins Pipeline
...
In this post also will explain how to consolidate all vulnerabilities/issues/bugs found and reports generated by all SAST Tools and Linters used in the CI/CD Pipeline.
...
In this Jenkins Pipeline I am going to use the next SAST Tools/Linters:
...
4. IaC (Terraform, K8s, CloudFormation, Dockerfiles, etc): [Kics](https://github.com/Checkmarx/kics), It already used in the [previous post](/2021/07/06/sast-in-sdlc-for-cloud-native-apps/)
...
If you want add more Linters because you added Java and NodeJS code, then, You can found other linters in this list: [Security along the SDLC for Cloud-Native Apps](/2020/02/10/security-along-the-container-based-sdlc/#oss-sec-list).
...
2. Consider standarize all SAST Tools and Linter Reports and findings in a single platform. Here I've used [Jenkins Warnings-NG](https://www.jenkins.io/doc/pipeline/steps/warnings-ng), take a look at it.
3. You should be aware that this (SAST) is only a part of DevSecOps. DAST, IASP, etc. have yet to be implemented. However, at this point, you already have your Cloud-Native Projects more secure.
